...
import dev.spring.petclinic.findowner.model.Owners;  // Owners 클래스를 import 해야 함
...
public class FindOwnerController {
    ...
    public String findOwnersPage(Model model) {
        model.addAttribute("owner", new Owners());  // owner 객체를 모델에 추가
    ...
    public String searchOwners(@RequestParam(value = "lastName", required = false) String lastName, Model model) {
        ...
        if (lastName == null || lastName.trim().isEmpty()) {
            ...
            ownersList = findOwnerService.findAllOwners();
        } else {
            // 검색어가 있으면 성(lastName)을 기준으로 검색
            ownersList = findOwnerService.findOwnersByLastName(lastName);
        }
